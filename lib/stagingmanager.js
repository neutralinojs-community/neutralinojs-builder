...
function findPlatformBinaries(appDistDir, targetPlatform) {
    ...
    const suffixes = PLATFORM_SUFFIXES[targetPlatform] || [];
...
function findAppDistDir(distDir, config) {
    ...
    const appDirs = entries.filter((entry) => {
        ...
        return (
            ...
            findPlatformBinaries(candidatePath, config.targetPlatform).length > 0
        );
    });
...
function prepare(config) {
    ...
    const binaries = findPlatformBinaries(appDistDir, config.targetPlatform);
